...
class Paginator implements PaginatorInterface
{
    ...
    public function queryBuilderForResult(QueryBuilder $queryBuilder, Request $request, array $criteria = []): QueryBuilder
    {
        ...
        $queryBuilderResult->setMaxResults($limit)
    ...
    {
        $dql = '';
        $parameters = $whereGrouped = [];
        $criteria = $this->sortCriteriaByRequest($criteria);

        foreach ($criteria as $field => $value) {
            [$alias, $_field] = ($this->isFieldNested($field) ? $this->getNestedAliasField($queryBuilder, $field, $rootAlias, true) : [$rootAlias, $field]);
    ...
    protected function getFilterWhere(string $filter, string $alias, string $field, ?string $value): array
    {
        ...
        return match ($filter) {
            'lk' => [self::AND, "$alias.$field LIKE :$parameter", $parameter, "%$value%"],
            'olk' => [self::OR, "$alias.$field LIKE :$parameter", $parameter, "%$value%"],
            'gt' => [self::AND, "$alias.$field > :$parameter", $parameter, $value],
            'ogt' => [self::OR, "$alias.$field > :$parameter", $parameter, $value],
            'gte' => [self::AND, "$alias.$field >= :$parameter", $parameter, $value],
            'ogte' => [self::OR, "$alias.$field >= :$parameter", $parameter, $value],
            'lt' => [self::AND, "$alias.$field < :$parameter", $parameter, $value],
            'olt' => [self::OR, "$alias.$field < :$parameter", $parameter, $value],
            'lte' => [self::AND, "$alias.$field <= :$parameter", $parameter, $value],
            'olte' => [self::OR, "$alias.$field <= :$parameter", $parameter, $value],
            'eq', 'oeq' => [(($filter === 'eq') ? self::AND : self::OR), "$alias.$field " . (is_null($value) ? 'IS NULL' : "= :$parameter"), $parameter, $value],
            'neq', 'oneq' => [(($filter === 'neq') ? self::AND : self::OR), "$alias.$field " . (is_null($value) ? 'IS NOT NULL' : "<> :$parameter"), $parameter, $value],
            default => throw new BadRequestHttpException(sprintf('Invalid filter "%s"', $filter))
        };
    }
    ...
    protected function addOrder(QueryBuilder $queryBuilder, array $order, string $rootAlias): void
    {
        foreach ($order as $field => $direction) {
            ...
            $queryBuilder->addOrderBy("$alias.$_field", strtoupper($direction));
        }
    }
